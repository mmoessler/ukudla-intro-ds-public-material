# Prepare the maize country-year data

---

- Last Update: 2026-08-21
- Source: [06_data_preparation_application.md](/learning-modules/intro-ds-module/06_data_preparation_application.md)
...
- inspect the FAOSTAT input without changing it;
- validate element-unit mappings and source-key uniqueness;
- reshape the long FAOSTAT values into a country-year panel;
- verify a unit conversion and a log transformation;
- explain which preparation occurs before and after integration;
- compare input and output coverage, keys, and missingness; and
...
Before beginning, review [Why prepare data?](06_data_preparation_motivation.md) and [Understand
data-preparation concepts](06_data_preparation_concepts.md).
...
Do not continue through a failed key, mapping, unit, or coverage expectation
only to obtain the expected output filename.
...
- Yield in <code>kg/ha</code>;
- Production in <code>t</code>; and
- Area harvested in <code>ha</code>.

The project needs one row per country and year with separate columns for those
measures. It also needs stable project identifiers and growing-season
precipitation from the preceding integration topic.
...
The source-specific maize panel is an intermediate derived artifact. The
integrated table is the current input to exploration and modeling.

For this exercise, the preparation evidence should establish:

- the input and output grain;
- source and output candidate keys;
- element-unit mappings;
- row-count and coverage expectations;
- unit conversion;
- derived-variable domain;
- missingness behavior;
- unchanged managed inputs; and
- lineage from source fields to output variables.
...
Predict which source columns will remain, which will become output column
names, and which will be represented through the output variable names and
units.
...
- nine areas;
- one item, <code>Maize (corn)</code>;
- years 1990–2022;
- three documented element-unit combinations; and
- 891 rows before reshaping.

Read the data dictionary and flag code list. A variable name alone is
insufficient to interpret <code>value</code> because its meaning and unit
...
Every observed pair must map exactly once. Do not use a catch-all label for an
...
this exercise, assemble the following checks in code or a table:
...
but currently lacks its own data dictionary and human-readable dataset page.
A complete preparation implementation should consider adding:
...
Avoid duplicating authoritative facts:

- Markdown explains purpose, construction, assumptions, and limitations;
- CSV defines output variables and domains;
- YAML records artifact history and transformations;
- scripts execute transformations; and
- audit tables record observed checks.
...
The current transformations are based on fixed source definitions:

- element-unit mapping;
- deterministic reshaping;
- unit conversion;
- country crosswalk;
- temporal alignment; and
- log formula with a fixed mathematical definition.
...
- feature selection;
- data-dependent bins;
- rare-category handling; or
- outlier thresholds.

These must be estimated using training data only and applied unchanged to the
test period. Do not add such operations to a general preparation script that
...
Run <code>git status</code> and confirm that the tracked teaching input was
checked out. Do not run a provider download automatically.

### Required columns are missing

Compare the file with its dictionary and provenance. A changed schema may mean
the wrong artifact is present or the source workflow has changed.

### An element-unit combination is unexpected

Do not map by element alone. Inspect the pair, provider metadata, and source
changes before updating the contract.

### The source key is duplicated

Do not call <code>distinct()</code> automatically. Determine whether the key
omits a classification, flag, unit, method, or revision dimension.

### Widening creates multiple values

The proposed target cell is not unique. Return to the input grain and mapping.

### The panel has fewer than 297 rows

Inspect missing country-year-element combinations. Do not create rows or fill
values until the absence is understood.

### The log contains infinite values

Inspect zero or negative yield. The safe transformation should not retain
infinite values.

### A rerun changes managed inputs

Stop. Preparation must write only derived artifacts. Restore the intended input
through version control and inspect the script paths.
...
3. What information is represented by the new column names after reshaping?
4. Why is division by 1000 a unit conversion rather than an arbitrary scaling?
5. Why retain yield on the original physical scale after creating log yield?
6. Which checks compare source meaning rather than only file structure?
7. Which preparation steps must occur before CHIRPS integration?
8. Which preparation could occur after integration?
9. What evidence is currently missing for the intermediate maize panel?
10. Which future preprocessing operations could leak test information?
