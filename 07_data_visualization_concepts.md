...
- Source: [07_data_visualization_concepts.md](/learning-modules/intro-ds-module/07_data_visualization_concepts.md)
...
[Why visualize data?](07_data_visualization_motivation.md) explains why visual choices
matter. This page provides the decision model used in [the maize visualization
application](07_data_visualization_application.md).
...
This sequence does not imply that visualization is linear. Exploratory work is
iterative: a plot can reveal a new question, a metadata issue, or a need for a
different representation. Record consequential changes in code rather than
making them only through an interactive interface.
...
For the integrated maize data, one row represents one project country and year,
including a growing season ending in that year. In a scatterplot with yield on
the vertical axis and precipitation on the horizontal axis, one point can
therefore represent one country-year.

In a plot of average yield by country, one point represents 33 country-year
rows summarized by a mean. That is a different analytical object.

Always be able to complete this sentence:
...
These are starting points, not rules that make a graphic valid automatically.
A boxplot can hide multimodality; a line can imply continuity between sparse
observations; a map makes area visually prominent even when area is not the
...
Use **colour hue** primarily to distinguish a manageable number of categories.
Use an ordered lightness scale for ordered or continuous values when the
palette supports it. Do not use an arbitrary rainbow scale: perceived colour
differences do not follow equal numeric differences and important transitions
may appear where none exist.
...
Use **shape** for few categories and provide enough mark size to recognize it.
Whenever possible, encode an important distinction redundantly through
position, label, shape, or line type rather than colour alone.
...
A line joining countries would be inappropriate because country order is not a
continuous sequence. A line joining annual observations within one country is
meaningful when the observations are grouped by country and ordered by year.
...
Fixed scales across facets support magnitude comparison. Free scales can reveal
within-panel patterns when groups have very different ranges, but they prevent
direct visual comparison of absolute levels. State which comparison matters.
...
Do not calculate a mean with `na.rm = TRUE` without also reporting how many
values were missing. A national mean and a mean across countries can imply
different weights. Visualization does not remove these statistical choices.
...
Bins trade precise locations for counts. Faceting can make cross-panel
...
Do not choose the most dramatic exploratory view after trying many alternatives
without acknowledging that selection. A communication graphic should follow a
defensible question, not merely maximize apparent difference.
...
spatial aggregation, irrigation, inputs, temperature, and many omitted factors
can affect a visible rainfall-yield pattern. Keep these limitations attached to
the visual story.
...
precipitation](07_data_visualization_application.md). You will formulate plot
