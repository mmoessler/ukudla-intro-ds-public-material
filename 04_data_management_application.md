# Manage the maize-yield data

---

- Last Update: 2026-08-03
- Source: [04_data_management_application.md](/learning-modules/intro-ds-module/04_data_management_application.md)
...
Before beginning, review [Why manage data?](04_data_management_motivation.md) and [Understand data-management concepts](04_data_management_concepts.md). The exercise assumes that you can explain dataset grain, candidate keys, metadata, a data dictionary, provenance, project organization, responsible use, and fitness for purpose.

The numbered steps apply those concepts. Do not continue through a failed expectation only to reach the end of the instructions. Inspect the evidence, explain the failure, and record what must be resolved.

---

## Scenario and deliverable

You have received a fixed FAOSTAT teaching extract containing maize production, harvested area, and yield observations for selected Southern African countries. Before anyone prepares, visualizes, or models these values, the project team must determine what the data mean and whether they match the expected structure.
...
Do not open and resave the raw file in spreadsheet software. Spreadsheet applications can alter dates, encodings, numeric precision, delimiters, and quoting even when no deliberate edit is made.
...
Check the source license and terms before deciding whether the raw file or derived outputs may be redistributed. Record the required citation and attribution. If data are personal, confidential, commercially sensitive, or location-sensitive, apply the project's access and retention rules instead of placing them in the general repository.
...
The second command reports the matching ignore rule when the file is ignored. No output means that no ignore rule matched; it does not mean that committing the file is appropriate.

Document the storage decision in the project README or data-management record. Another contributor should not need to infer whether a missing raw file must be downloaded, requested, mounted, or restored from an archive.
...
Compare the observed results with the intended countries and years. Do not hard-code an expectation until you can explain where it came from.
...
Do not turn every warning into an automatic deletion. Preserve the evidence and document the reasoning required for later preparation.
...
Inspect `names(maize_raw)` and adapt the exercise to the actual source schema. Record the actual names in the dictionary. Do not rename raw-file columns inside the raw file.

### Problem 02: The candidate key is not unique

Inspect all rows for several duplicated key combinations. Look for omitted dimensions or statuses. Consult the metadata before changing the key or removing rows.

### Problem 03: Numbers were imported as text

Inspect the problematic values and the import specification. Possible causes include non-numeric symbols, decimal conventions, or provider-specific missing codes. Document the source representation; conversion belongs in a derived step.

### Problem 04: The checksum differs from another learner's

Compare file sizes, source versions, access dates, and retrieval methods. Do not assume either file is authoritative until their origins are established.
...
- [Research Data Management Checklist — The Turing Way](https://book.the-turing-way.org/reproducible-research/rdm/rdm-checklist.html) summarizes practical actions for raw-data preservation, backups, planning, documentation, standards, and institutional requirements.
- [Data Cleaning — The Turing Way](https://book.the-turing-way.org/reproducible-research/rdm/rdm-cleaning/) discusses raw-data backups, validation, and reproducible cleaning. Use it as a bridge to the later Data Preparation session; do not silently apply cleaning decisions during this application.
- [Data management checklist — UK Data Service](https://ukdataservice.ac.uk/learning-hub/research-data-management/plan-to-share/checklist/) provides questions for reviewing responsibilities, documentation, storage, access, preservation, and sharing.

### Organization and documentation

- [Organising data — UK Data Service](https://ukdataservice.ac.uk/learning-hub/research-data-management/format-your-data/organising/) gives practical guidance on folder structures and meaningful file names.
- [Study-level documentation — UK Data Service](https://ukdataservice.ac.uk/learning-hub/research-data-management/document-your-data/study-level-documentation/) provides a checklist for documenting purpose, methods, structure, coverage, processing, quality assurance, access, and licensing.
- [Data documentation: quantitative data — UK Data Service](https://ukdataservice.ac.uk/learning-hub/research-data-management/document-your-data/data-level/data-documentation-quantitative-data/) supports the data-dictionary task with guidance on variable descriptions, units, value labels, missing values, and codebooks.

### Working with tabular data in R

- [Starting with data — Data Carpentry](https://lessons.datacarpentry.org/R-ecology-lesson/02-starting-with-data.html) is a practical introduction to importing CSV files, inspecting data frames, and understanding observations and variables in R.
- [Data Analysis and Visualisation in R for Ecologists — Data Carpentry](https://lessons.datacarpentry.org/R-ecology-lesson/) provides a broader hands-on sequence for project organization, importing, manipulating, and analyzing ecological tabular data.
