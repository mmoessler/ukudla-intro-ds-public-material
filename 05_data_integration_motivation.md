...
- Source: [05_data_integration_motivation.md](/learning-modules/intro-ds-module/05_data_integration_motivation.md)
...
[Understand data-integration concepts](05_data_integration_concepts.md) develops the mental
model. [Integrate maize-yield and precipitation data](05_data_integration_application.md)
...
Combining sources can enrich an analysis, but it also introduces choices. A result can change because of the selected provider, query, release, country crosswalk, time aggregation, spatial boundary, join type, or unit conversion.

---

## Acquisition supports integration

Acquisition belongs to the data lifecycle introduced under Data Management.
It occurs for the original FAOSTAT source and again when CHIRPS is added. In
this session, it supports the main integration problem: obtaining an
additional source in a form that can be aligned with existing evidence.
...
An acquisition script should make these choices visible. A manual download can also be documented reproducibly when automation is unavailable, but every selection and interaction must be recorded.
...
The output is a derived country-year dataset plus a join audit. It can support
an analysis of associations, but does not show that precipitation causes
maize-yield differences.
...
Continue with [Understand data-integration concepts](05_data_integration_concepts.md). It
explains how to evaluate an additional source, understand representations,
align grains and identifiers, state join relationships, audit results, and
preserve lineage.
